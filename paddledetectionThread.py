...
class DetectionThread(object):
    def __init__(self):
        self.cfg = getattr(importlib.import_module('paddledetection.configs.paddledetection'), 'cfg')
        self.detector = Detector(
            self.cfg.model_path, use_gpu=self.cfg.gpu, run_mode=self.cfg.run_mode)


    def detectOnePicture(self,img):
        frame = img.copy()
        results = self.detector.predict(frame, 0.5)
